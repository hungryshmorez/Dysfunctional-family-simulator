...
describe('middle-child household',()=>{
  ...
  it('migrates old saves without changing their life or guessing gender',()=>{const s=newLife('Taylor');const {family,gender,...old}=s;expect(family).toBeDefined();expect(gender).toBeNull();const parsed=parseLife(old)!;expect(parsed.name).toBe('Taylor');expect(parsed.money).toBe(s.money);expect(parsed.family.birthOrder[1]).toBe('self');expect(parsed.gender).toBeNull();expect(parseLife({...s,family:{}})).toBeNull();});
